...
> Feedback: First of all, very nice visuals and coding style. You say "which leads me to believe that Solidity is not normally distributed". All three variables are not normally distributed, what you try to mean is the standard ways to transform the data to make it normally distributed are not applicable to Solidity. Creating nice visuals and functions are good, but spending too much time may cause you to overlook some important points.
...
> mcd25 \<- covMcd(perimiter.minor_axis, alpha = 0.25) should be alpha = 0.75. Your mcd25 has 75% breakdown value, i.e. very very robust. However, the good thing is in your discussion you show that you know it is robust. Your only mistake is not knowing the definition of breakdown value. distance-distance plot you commented out: plot(mcd25, which="distance", cutoff=sqrt(qchisq(0.99,2)), classic=TRUE) also gives another insights.
...
> When choosing how many PCs to keep, you can also use other useful plots like "corrplot::corrplot(cor(variables, scores))" (See, Exercises for Chapter 4 - Question 1.3) You can use PcaHubert(mytrainingdata, scale=mad, k = ncol(mytrainindata)) to take a look at all the PCs. Using classical PCA to decide the number of PCs is not the correct way.
...
> Feedback: Well done! Another important observation could be: RMSE on validation increases more compared to ME. This can be because the (possible) outliers in validation effect RMSE more than ME.

> Final Feedback: you made great visuals and nice functions. You can also check the other plots that are described in the class (You can check slides and assignments). The instructor of this course also likes creating nice and useful visuals. The only thing I can suggest is not to be lost in when creating generalized functions and plots. They are not always time-saving. Being proficient in R is also about knowing when to use which function and how to use it effectively. Finally, the main goal should be knowing the statistical methodology, don't skip that part.
